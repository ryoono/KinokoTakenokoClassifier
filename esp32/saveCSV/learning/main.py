...
CSV_FILE = "data_with_labels_raw.csv"
...
# 多クラスロジスティック回帰の学習（multi_class='multinomial'、solver='lbfgs'を指定）
...
# 学習した係数（クラス数 x 特徴量数）と切片（各クラス毎）
print("係数:\n", clf.coef_)
print("切片:\n", clf.intercept_)

# 学習済みモデルの保存（後で参照・評価する場合などに利用）
joblib.dump(clf, "logistic_model.pkl")
print("学習済みモデルを 'logistic_model.pkl' として保存しました。")
